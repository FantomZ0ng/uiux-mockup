  ...
  return (
    <div className='p-10 md:px-48 xl:px-60 mt-8 flex flex-col items-center justify-center'>
        <div className='flex items-center justify-center w-full'>
            ...
            <h2 className='text-5xl font-bold text-center'>Design High Quality <span className='text-primary'>Webdite and Mobile App</span>  Designs</h2>
            <p className='text-center text-gray-600 text-lg mt-3'>Imegine your idea and turn to reality</p>
    ...
    <div className='flex gap-3 mt-4'>
        {suggestions.map((suggestions,index)=>(
            <div key={index} className='p-2 border rounded-2xl
             ...
             onClick={()=>setUserInput(suggestions?.description)}>
                <h2 className='text-lg'>{suggestions?.icon}</h2>
                <h2 className='text-center line-clamp-2 text-sm'>{suggestions?.name}</h2>
            </div>
        ))}
    </div>
    </div>
